...
export function AnalysisDashboard({ videoUrl, analysis: initialAnalysis }) {
  ...
  const groqScoreBreakdown = analysis.scoreBreakdown || null;
    ...
    <>
      <div
        style={{
          ...
          backgroundImage: 'radial-gradient(circle at 50% 50%, #0F172A 0%, #020617 100%) 100%)',
          color: '#F8FAFC',
          minHeight: '100vh',
        ...
        <main style={{ maxWidth: '85rem', margin: '0 auto', padding: '2rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.5fr) minmax(0, 1fr)', gap: '2rem', alignItems: 'center' }}>
                  <div style={{ display: 'grid', gap: '1.25rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
                      <div>
                        <p style={{ margin: 0, fontSize: '0.85rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#94A3B8', fontWeight: 700 }}>Coding Performance Dashboard</p>
                        <h2 style={{ margin: '0.25rem 0 0 0', fontSize: '1.75rem', fontWeight: 800, color: '#F8FAFC', letterSpacing: '-0.02em' }}>Interview Score Insights</h2>
                      </div>
                      <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.6rem 1rem', borderRadius: '9999px', backgroundColor: '#1E293B', color: '#fff', fontWeight: 700, fontSize: '0.95rem', boxShadow: '0 4px 12px rgba(15,23,42,0.2)' }}>
                        <Gauge size={18} /> Overall {overallScore}
                      </div>
                    </div>

                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: '1rem' }}>
                      <div style={{ borderRadius: '1rem', padding: '1.25rem', backgroundColor: '#0F172A', border: '1px solid rgba(99,102,241,0.1)', boxShadow: '0 4px 12px -4px rgba(99,102,241,0.1)' }}>
                        <p style={{ margin: 0, fontSize: '0.8rem', color: '#6366f1', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Overall</p>
                        <p style={{ margin: '0.25rem 0 0 0', fontSize: '1.5rem', fontWeight: 800, color: '#F8FAFC' }}>{overallScore}<span style={{ fontSize: '0.9rem', color: '#94a3b8' }}>/100</span></p>
                      </div>
                      <div style={{ borderRadius: '1rem', padding: '1.25rem', backgroundColor: '#0F172A', border: '1px solid rgba(16,185,129,0.1)', boxShadow: '0 4px 12px -4px rgba(16,185,129,0.1)' }}>
                        <p style={{ margin: 0, fontSize: '0.8rem', color: '#10b981', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Average</p>
                        <p style={{ margin: '0.25rem 0 0 0', fontSize: '1.5rem', fontWeight: 800, color: '#F8FAFC' }}>{averageMetric}<span style={{ fontSize: '0.9rem', color: '#94a3b8' }}>/100</span></p>
                      </div>
                      <div style={{ borderRadius: '1rem', padding: '1.25rem', backgroundColor: '#0F172A', border: '1px solid rgba(245,158,11,0.1)', boxShadow: '0 4px 12px -4px rgba(245,158,11,0.1)' }}>
                        <p style={{ margin: 0, fontSize: '0.8rem', color: '#f59e0b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Strongest</p>
                        <p style={{ margin: '0.25rem 0 0 0', fontSize: '1.15rem', fontWeight: 800, color: '#F8FAFC', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{strongestMetric?.label ?? 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ borderRadius: '1.25rem', padding: '1.5rem', backgroundColor: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.5)', boxShadow: '0 4px 24px -8px rgba(0,0,0,0.05)' }}>
                    <p style={{ margin: 0, fontSize: '0.8rem', color: '#94A3B8', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', textAlign: 'center', marginBottom: '1.25rem' }}>Metric Snapshot</p>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: '1rem' }}>
                    ...
                    <div key={item.key} style={{ display: 'grid', justifyItems: 'center', gap: '0.5rem' }}>
                        ...
                        style={{
                          ...
                          height: '64px',
                          borderRadius: '9999px',
                          background: `conic-gradient(${item.color} ${item.value}%, #334155 0%)`,
                          display: 'flex',
                          alignItems: 'center',
                          ...
                          boxShadow: '0 4px 10px rgba(0,0,0,0.05)'
                            ...
                            <div style={{ width: '48px', height: '48px', borderRadius: '9999px', backgroundColor: '#0F172A', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.85rem', fontWeight: 800, color: '#F8FAFC' }}>
                              {item.value}
                            </div>
                          </div>
                          <span style={{ fontSize: '0.75rem', textAlign: 'center', lineHeight: 1.3, fontWeight: 700, color: '#E2E8F0' }}>{item.label}</span>
                        </div>
                    )}
                    </div>
                  </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2rem', alignItems: 'flex-start' }}>
            <div style={{ flex: '1 1 44rem', display: 'grid', gap: '2rem' }}>
              <Card style={{ overflow: 'hidden', borderRadius: '1.25rem', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05), 0 2px 4px -2px rgba(0,0,0,0.025), inset 0 1px 0 rgba(255,255,255,0.5)', border: '1px solid rgba(226, 232, 240, 0.8)', backgroundColor: '#0F172A' }}>
                <CardContent style={{ padding: 0 }}>
                  <video
                    controls
                    src={videoUrl}
                    style={{ width: '100%', aspectRatio: '16/9', display: 'block', backgroundColor: '#1E293B', borderRadius: '1.25rem 1.25rem 0 0' }} />
                  
                </CardContent>
              </Card>

              {hasGroqOverallScores &&
              <Card style={{ borderRadius: '1.25rem', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05), 0 2px 4px -2px rgba(0,0,0,0.025), inset 0 1px 0 rgba(255,255,255,0.5)', border: '1px solid rgba(226, 232, 240, 0.8)', overflow: 'hidden', backgroundColor: '#0F172A', transition: 'all 0.3s ease' }}>
                <CardHeader>
                ...
                <CardContent>
                  <div style={{ marginBottom: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ fontSize: '1.125rem', fontWeight: 800, color: 'hsl(var(--primary))' }}>{analysis.score ?? 0}</span>
                  </div>
              ...
              <Card style={{ borderRadius: '1.25rem', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05), 0 2px 4px -2px rgba(0,0,0,0.025), inset 0 1px 0 rgba(255,255,255,0.5)', border: '1px solid rgba(226, 232, 240, 0.8)', overflow: 'hidden', backgroundColor: '#0F172A', transition: 'all 0.3s ease' }}>
                  ...
                  <CardContent>
                    ...
                    <div style={{ display: 'grid', gap: '0.75rem' }}>
                      {[
                    { label: 'Code Quality', value: analysis.scoreBreakdown.codeQuality, color: '#6366f1' },
                    { label: 'Problem Solving', value: analysis.scoreBreakdown.problemSolving, color: '#10b981' },
                    { label: 'Visual Confidence', value: analysis.scoreBreakdown.visualConfidence, color: '#f59e0b' },
                    { label: 'Grammar', value: analysis.scoreBreakdown.grammar, color: '#ef4444' },
                    { label: 'Fluency', value: analysis.scoreBreakdown.fluency, color: '#0ea5e9' }].
                    map((item) =>
              ...
              {Array.isArray(analysis.questionByQuestionAnalysis) && analysis.questionByQuestionAnalysis.length > 0 &&
              <Card style={{ borderRadius: '1.25rem', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05), 0 2px 4px -2px rgba(0,0,0,0.025), inset 0 1px 0 rgba(255,255,255,0.5)', border: '1px solid rgba(226, 232, 240, 0.8)', overflow: 'hidden', backgroundColor: '#0F172A', transition: 'all 0.3s ease' }}>
                  <CardHeader>
                    <CardTitle style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <div style={{ flex: '1 1 22rem', display: 'grid', gap: '2rem' }}>
              ...
              <Accordion type="multiple" defaultValue={['content-analysis', 'video-analysis', 'vocal-analysis']}>
                {hasContentAnalysis &&
                <Card style={{ borderRadius: '1.25rem', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05), 0 2px 4px -2px rgba(0,0,0,0.025), inset 0 1px 0 rgba(255,255,255,0.5)', border: '1px solid rgba(226, 232, 240, 0.8)', overflow: 'hidden', backgroundColor: '#0F172A', transition: 'all 0.3s ease', marginBottom: '1rem' }}>
